...
use pichost_core::state::{CacheError, InviteError, InviteVerifyStatus};
use serde::Serialize;
use uuid::Uuid;
...
/// Generic over any Serde-compatible type and any `Cache` impl.
pub async fn cached_meta<C, T, F, E>(
    cache: &C,
    image_id: &uuid::Uuid,
    ttl: u64,
    fetch_fn: F,
...
where
    C: pichost_core::state::Cache + ?Sized,
    ...
    F: std::future::Future<Output = Result<T, E>>,
{
    ...
    if let Ok(json) = serde_json::to_string(&val) {
        let _ = cache.set_ex(&key, &json, ttl).await;
    }
...
pub async fn cached_thumb<C, F, E>(
    cache: &C,
    cache_key: &str,
    ttl: u64,
    fetch_fn: F,
    ...
    F: std::future::Future<Output = Result<Vec<u8>, E>>,
{
    ...
    // Try cache hit — read raw bytes directly (not via `get` which does String)
    if let Ok(Some(bytes)) = cache.get_bytes(&redis_key).await {
        return Ok(bytes);
    }
    ...
    // Store (best-effort)
    let _ = cache.set_ex_bytes(&redis_key, &bytes, ttl).await;
...
impl pichost_core::state::Cache for Cache {
    ...
    async fn incr_user_stat(
        ...
        self.incr_user_stat(user_id, field, delta)
            .await
            .map_err(|e| CacheError::Other(e.to_string()))
    }
    ...
    async fn set_user_stats(
        ...
        self.set_user_stats(user_id, fields)
            .await
            .map_err(|e| CacheError::Other(e.to_string()))
    }
...
impl pichost_core::state::InviteStore for RedisInviteStore {
    ...
    async fn verify_detail(&self, code: &str) -> Result<InviteVerifyStatus, InviteError> {
        let status = self
            ...
            .map_err(|e| InviteError::Other(e.to_string()))?;
        Ok(match status {
            InviteVerifyResult::Valid => InviteVerifyStatus::Valid,
            InviteVerifyResult::Used => InviteVerifyStatus::Used,
            InviteVerifyResult::Expired => InviteVerifyStatus::Expired,
            InviteVerifyResult::NotFound => InviteVerifyStatus::NotFound,
        })
    }
}
...
mod tests {
    ...
    async fn test_ops_fail_with_unreachable_redis() {
        ...
        assert!(cache
            .set_user_stats(&Uuid::new_v4(), &[("f", Some(1))])
            .await
            .is_err());
        ...
        assert!(cache
            .consume_invite_code("x", &Uuid::new_v4())
            .await
            .is_err());
    ...
    async fn test_cached_fallbacks_with_unreachable_redis() {
        ...
        let meta: Result<(i32,), std::io::Error> =
            cached_meta(&cache, &Uuid::new_v4(), 60, async { Ok((7,)) }).await;
        assert_eq!(meta.unwrap().0, 7);
        let thumb: Result<Vec<u8>, std::io::Error> =
            cached_thumb(&cache, "k", 60, async { Ok(vec![1u8, 2]) }).await;
