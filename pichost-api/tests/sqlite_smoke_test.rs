...
async fn sqlite_admin_stats_queries() {
    ...
    let q: (i64, i64) =
        sqlx::query_as("SELECT COUNT(*), CAST(COALESCE(SUM(file_size), 0) AS BIGINT) FROM images")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(q, (0, 0));
    ...
    let n: i64 =
        sqlx::query_scalar("SELECT COUNT(DISTINCT user_id) FROM images WHERE created_at >= ?")
            .bind(cutoff)
            .fetch_one(&pool)
            .await
            .unwrap();
