...
fn test_image_list_query_parse_all_params() {
    let query: ImageListQuery =
        serde_urlencoded::from_str("page=2&per_page=10&sort=file_size&order=asc&search=cat")
            .unwrap();
...
fn test_batch_move_request_serde() {
    let json = concat!(
        ...
        r#""category_id":"00000000-0000-0000-0000-000000000003"}"#
    );
...
fn test_image_list_query_with_category_id() {
    ...
    let params: ImageListQuery = serde_urlencoded::from_str(query).unwrap();
