...
- 安装包发布: release CI 将全部安装包(tar.gz/deb/rpm/exe)附加到 GitHub Release;`scripts/publish-repo.sh`/`scripts/setup-repo.sh` 与 `packaging/homebrew/pichost.rb.tpl` 脚本保留但 **CI 不再自动推送** apt/rpm 仓库或 Homebrew tap。
