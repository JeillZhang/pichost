...
P4 引入 9 个新特性领域，分为独立阶段。各阶段在 P4-A 完成后可独立开发和部署。
...
- 管理员可在 Web UI 中配置数据库连接、Redis 连接等系统级参数
- 配置优先级：**UI 设置 > config.toml > 环境变量 > 代码默认值**
- UI 设置写入 `config.toml` 文件，重启后生效
- 提供配置校验（数据库/Redis 连接测试）、备份、恢复功能

### 10.2 架构决策：UI → config.toml

DB 和 Redis 的连接信息不能存于数据库本身（循环依赖）。方案对比如下：

| 方案 | 描述 | 评价 |
|------|------|------|
| **UI → config.toml** ✅ | 管理页编辑后直接写入 `config.toml`，重启生效 | 无循环依赖，可审计，可备份 |
| UI → 环境变量文件（`.env`） | 写入 `.env` | 比 config.toml 更脆弱，toml 有结构校验 |
| UI → 独立系统设置表 | 存入 DB 的 `system_settings` 表 | 循环依赖：修改 DB URL 后无法重连 |
| 热重载（不重启） | 运行时动态重建连接池 | 复杂，风险高（正在处理的请求会断开） |

**选定**: UI → config.toml + 重启生效。这是最稳健的方案。

### 10.3 配置优先级（Figment 改造）

当前 figment 配置优先级为：`defaults → config.toml → PICHOST_* env`。

改造后优先级调整为：

```rust
// figment 合并顺序（后面的覆盖前面的）
Figment::new()
    .merge(Serialized::defaults(Config::default()))      // 1. 代码默认值
    .merge(Toml::file("config.toml"))                     // 2. config.toml（UI 写入）
    .merge(Env::prefixed("PICHOST_"))                     // 3. 环境变量（运维覆盖）
```

即 **env 仍为最高优先级**，用于紧急运维覆盖。UI 写入的 config.toml 高于代码默认值。管理员日常运维通过 UI 修改 config.toml，docker/k8s 环境仍通过 env 注入。

> **用户界面设置的含义**：若用户通过 UI 设置了一个值，该值写入 config.toml，重启后生效。若同时存在同名环境变量，环境变量优先（docker/k8s 场景）。若管理员希望 UI 设置完全覆盖 env，可删除环境变量。
...
| Method | Path | 说明 |
|--------|------|------|
| `GET` | `/api/v1/admin/config` | 获取当前系统配置（敏感字段掩码） |
| `PUT` | `/api/v1/admin/config` | 更新系统配置并写入 config.toml。Body: `{ "database_url": "...", "redis_url": "..." }` |
| `POST` | `/api/v1/admin/config/test` | 测试配置连接。Body: `{ "database_url": "...", "redis_url": "..." }`。返回连接测试结果。 |
| `POST` | `/api/v1/admin/config/backup` | 备份当前 config.toml → `config.toml.{timestamp}.bak` |
| `POST` | `/api/v1/admin/config/restore` | 从备份恢复。Body: `{ "backup_file": "config.toml.2026-07-19T12:00:00Z.bak" }` |
| `GET` | `/api/v1/admin/config/backups` | 列出所有备份文件 |

**配置项（可管理）**：

| 字段 | 说明 | 掩码规则 |
|------|------|----------|
| `database_url` | PostgreSQL 连接字符串 | `postgresql://user:***@host:port/db` |
| `redis_url` | Redis 连接字符串 | `redis://:***@host:port` |
| `auth.jwt_secret` | JWT 签名密钥 | 完全掩码 `********` |
| `auth.token_encryption_key` | PAT 加密密钥 | 完全掩码 `********` |
| `server.public_url` | 公开访问 URL | 不掩码 |
| `storage.default_backend` | 默认存储后端 | 不掩码 |
| `storage.local_base_path` | 本地存储路径 | 不掩码 |

### 10.5 前端设计

Admin 页面新增 **"系统配置"** Tab（与 Overview / Users / Invites 并列）：
...
**交互规则**：
- 每个配置项旁有「测试连接」按钮（DB/Redis）
- 修改任何字段后，「保存」按钮激活
- 保存成功后弹出 toast：*"配置已保存至 config.toml，请重启服务使其生效"*
- 记录 git diff 展示本次修改内容
- JWT 密钥重置需二次确认

### 10.6 实现要点

**后端读写 config.toml**：
- 使用 `toml` crate 序列化/反序列化
- 写入前自动备份（`config.toml → config.toml.{timestamp}.bak`）
- 保留注释和格式（使用 `toml_edit` crate 而非 `toml`，支持 preserve 格式）
- 写入后 fsync 确保落盘

**校验逻辑**：
- `database_url`: 尝试 `sqlx::PgPool::connect()`，超时 5s
- `redis_url`: 尝试 `redis::Client::open()` + `PING`，超时 3s
- `jwt_secret`: 校验长度 ≥ 32 字符
- `public_url`: 校验合法 URL 格式

**安全**：
- 只有 Admin 角色可访问
- 操作记录审计日志（谁、何时、改了什么）
- DB/Redis 连接字符串中的密码掩码返回
...
| 错误的 DB URL 写入 config.toml 导致服务无法启动 | 中 | 高 | 写入前**强制测试连接**，测试失败拒绝保存；自动备份上一份有效配置；提供 CLI 恢复工具 |
| config.toml 被手动编辑后格式损坏 | 低 | 中 | 使用 `toml_edit` 保留格式；写入前校验语法；所有写入前自动备份 |
| 管理员误改 JWT secret 导致所有用户登出 | 低 | 中 | 重置 JWT 密钥需二次确认 + 警告提示；备份按钮醒目 |
