...
test.describe.serial("settings", () => {
  ...
  test("profile section shows current values and saves", async ({
    page,
    request,
  }) => {
  ...
  test("password change with short new password rejected client-side", async ({
    page,
    request,
  }) => {
  ...
  test("storage backends section shows empty state and add modal opens", async ({
    page,
    request,
  }) => {
    const settings = new SettingsPage(page);
    ...
    await expect(
      page.getByRole("heading", { name: /storage configs/i }),
    ).toBeVisible();
  ...
  test("creating a storage config without a token is rejected", async ({
    page,
    request,
  }) => {
    const settings = new SettingsPage(page);
  ...
  test("preprocessing section toggles EXIF stripping", async ({
    page,
    request,
  }) => {
    const settings = new SettingsPage(page);
  ...
  test("oauth section shows provider links", async ({ page, request }) => {
    ...
    await expect(
      page.getByRole("link", { name: /link github/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: /link google/i }),
    ).toBeVisible();
  });
});
