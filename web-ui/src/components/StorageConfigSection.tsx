...
function ConfigModal({ editing, onClose }: ConfigModalProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : t("storageConfig.saveFailed"),
      );
    } finally {
      setSaving(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input
            ...
            placeholder={
              form.provider === "github"
                ? t("storageConfig.namePlaceholderGithub")
                : t("storageConfig.namePlaceholderGitcode")
            }
            value={form.name}
        ...
        <div>
          <label
            ...
            style={{ color: "var(--color-text-secondary)" }}
          >
            {t("storageConfig.token")}{" "}
            {isEdit && t("storageConfig.tokenKeepHint")}
          </label>
          <div className="relative">
            <input
              ...
              placeholder={
                isEdit
                  ? t("storageConfig.tokenPlaceholderEdit")
                  : t("storageConfig.tokenPlaceholderNew")
              }
              value={form.token}
...
function DeleteConfirm({ config, onClose }: DeleteConfirmProps) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : t("storageConfig.deleteFailed"),
      );
    } finally {
      setDeleting(false);
    }
...
export default function StorageConfigSection() {
  ...
  const [editingConfig, setEditingConfig] = useState<UserStorageConfig | null>(
    null,
  );
  const [deletingConfig, setDeletingConfig] =
    useState<UserStorageConfig | null>(null);
  ...
  async function handleSetDefault(id: string) {
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : t("storageConfig.setDefaultFailed"),
      );
    }
  }
  ...
  return (
    <div className="relative">
      ...
      {modalOpen && (
        <ConfigModal editing={editingConfig} onClose={closeModal} />
      )}
      {deletingConfig && (
        <DeleteConfirm
